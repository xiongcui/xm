<template>
  <view>
    <view @tap="sendMessage_TIM('113553', 'shuju')">发送</view>
    <!-- 
     ...
     -->
        <block v-for="(item, index) in msgList" :key="index">
          <view
            ...
            class="right-chat-info flex-box-end"
            :id="'msg-' + index"
          >
            <view class="right-chat-msg">{{ item.payload.text }}</view>
            <view class="right-chat-head"
              ><image
                class="chat-headPortrait"
          ...
          <view
            wx:else
            class="left-chat-info flex-box-start"
            :id="'msg-' + index"
          >
            <view class="left-chat-head"
              ><image
                class="chat-headPortrait"
...
export default {
  ...
  methods: {
    ...
    sendMessage_TIM(sendTo, msg) {
      //这里是文字消息的发送,有需求可以增加个type参数控制发送方式,文字/图片/音视频/文件 调用不同的tim接口
      let message = this.$TUIKit.createTextMessage({
        to: sendTo,
      ...
      let promise = this.$TUIKit.sendMessage(message);
      promise
        .then(function (imResponse) {
          // 发送成功
          console.log("发送成功");
